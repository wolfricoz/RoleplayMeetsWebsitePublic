<template>
  <div class="w-full rounded-xl">
    ...
    <Multiselect
      :classes="{
  container: 'relative rounded-xl mx-auto w-full flex items-center justify-end box-border cursor-pointer rounded bg-white text-base leading-snug outline-none dark:bg-gray-600 dark:text-gray-200',
  wrapper: 'relative rounded-xl mx-auto w-full flex items-center justify-end box-border cursor-pointer outline-none dark:bg-gray-600 dark:text-gray-200',
  ...
  spacer: 'h-9 py-px box-content'
      ...
      :max=max
    />
    <p class="text-gray-500 text-sm text-right p-2" v-show="max !== -1"> {{ selected_values.length }} / {{ max }}</p>
  </div>
</template>

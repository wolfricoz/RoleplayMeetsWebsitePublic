<template>
  <div class="w-full rounded-xl">
    ...
    <Multiselect
      :classes="{
        container:
          'relative rounded-xl mx-auto w-full flex items-center justify-end box-border cursor-pointer rounded bg-white text-base leading-snug outline-none dark:bg-gray-600 dark:text-gray-200',
        wrapper:
          'relative rounded-xl mx-auto w-full flex items-center justify-end box-border cursor-pointer outline-none dark:bg-gray-600 dark:text-gray-200',
        ...
        dropdown:
          'max-h-60 absolute -left-px -right-px bottom-0 transform translate-y-full border border-gray-300 -mt-px overflow-y-scroll z-50 bg-white flex flex-col rounded-b dark:bg-gray-600 dark:text-gray-200',
        dropdownTop:
      ...
      :max="max"
    />
    <p class="p-2 text-right text-sm text-gray-500" v-show="max !== -1">
      {{ selected_values.length }} / {{ max }}
    </p>
  </div>
</template>
...
export default {
  ...
  beforeMount() {
    ...
    if (this.selected != null) {
      ...
      for (let i = 0; i < this.selected.length; i++) {
        ...
        if (
          this.selected[i].name === undefined &&
          typeof this.selected[i] === "string"
        ) {
  ...
  computed: {
    selectedCount() {
      return this.selected_values.length;
    },
  },
};
</script>

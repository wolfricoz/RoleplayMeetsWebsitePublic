    ...
    @if ($post->approved)
      <form action="{{ route("admin.approve", $post) }}" method="post">
        ...
        <x-admin-layout.cms_form_button
          name="disapprove"
          value="false"
          class="w-full border-red-700 text-left hover:bg-red-600 dark:text-red-400 dark:hover:bg-red-700"
    ...
    @else
      <form action="{{ route("admin.approve", $post) }}" method="post">
        ...
        <x-admin-layout.cms_form_button
          name="approve"
          value="true"
          class="hover:bg-green-60 w-full border-green-700 text-left dark:text-green-400 dark:hover:bg-green-700"
      ...
      <createmodal :button="'Deny'" :title="'Deny reason'">
        <form method="POST" action="{{ route("admin.reject", $post) }}">
          ...
          <x-admin-layout.cms_form_button
            class="w-32 border-red-700 text-left hover:bg-red-600 dark:text-red-400 dark:hover:bg-red-700"

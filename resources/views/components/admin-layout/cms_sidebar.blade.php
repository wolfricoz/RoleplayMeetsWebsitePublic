<aside class="flex flex-col justify-between w-48 h-full bg-gray-200">
    ...
    <div>
        ...
        <x-admin-layout.cms_button href="{{ route('admin.dashboard') }}"
                                   :permission="auth()->user()->group->access_dashboard">
            Dashboard
        ...
        <x-admin-layout.cms_button :permission="auth()->user()->group->manage_genres" :display="$genresCount">
            Genres
        </x-admin-layout.cms_button>

        <x-admin-layout.cms_button :permission="auth()->user()->group->manage_users" :display="$usersCount">
            Users
        </x-admin-layout.cms_button>
        ...
        <x-admin-layout.cms_button href="{{ route('admin.groups') }}" :permission="auth()->user()->group->manage_groups"
                                   :display="$groupsCount">
            Groups
        </x-admin-layout.cms_button>
        ...
        <x-admin-layout.cms_button :permission="auth()->user()->group->manage_settings">
            Settings
        </x-admin-layout.cms_button>

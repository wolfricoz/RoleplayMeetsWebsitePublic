<x-layout.header>
  <div id="app" class="m-1 flex justify-center lg:m-4">
    ...
    >
      ...
      <form
        method="post"
        action="{{ route("posts.report.store", $post) }}"
      ...
      >
        ...
        <select
          name="reason"
          id="reason"
          class="mt-1 block h-9 w-full rounded-xl p-1 px-2 dark:bg-gray-600 dark:text-gray-200"
          required
        >
          @foreach ($report_reasons as $reason)
            <option
              value="{{ $reason }}"
              {{ old("reason") === $reason ? "selected" : "" }}
            >
              {{ $reason }}
            </option>
          @endforeach
        ...
        <p class="pt-2 text-sm text-gray-500">
          ...
          and you acknowledge that the information you've provided is factual.
          If you report a post that is not in violation of the terms of service
          or rules, we reserve the right to take action against your account.
        </p>
        @if ($errors->any())
